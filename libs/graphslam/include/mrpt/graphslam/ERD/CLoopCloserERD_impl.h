				...
				for (vector_uint::const_iterator
					...
					for (vector_uint::const_iterator
						...
						MRPT_LOG_DEBUG_STREAM <<
							...
							b1 << ", " << b2 << ", " << a1 << ", " << a2 <<
							" => " << consistency << endl;
						hypots_to_consistencies[make_pair(hypot_b2_a1, hypot_b1_a2)] = consistency;
					}
				}
			}
		}
		...
		if (!w.isZero()) {
			...
			for (int wi = 0; wi != w.size(); ++wi) {
				if (w(wi) == 1)  {
					...
					hypot_t* hypot = this->findHypotByID(hypots_pool, wi);
					this->registerHypothesis(*hypot);
				}
			}
		}
		//mrpt::system::pause();
...
double CLoopCloserERD<GRAPH_t>::generatePWConsistencyElement(
	...
	TUncertaintyPath<GRAPH_t>* path_a1_a2 = this->queryOptimalPath(a2);
	...
	TUncertaintyPath<GRAPH_t>* path_b1_b2 = this->queryOptimalPath(b2);
